...
import Avatar from "./zoomComponents/avatar";
import VideoFooter from "./zoomComponents/video-footer";
import Pagination from "./zoomComponents/pagination";
import { useCanvasDimension } from "./zoomHooks/useCanvasDimension";
import { useGalleryLayout } from "./zoomHooks/useGalleryLayout";
import { usePagination } from "./zoomHooks/usePagination";
import { useActiveVideo } from "./zoomHooks/useAvtiveVideo";
import { useShare } from "./zoomHooks/useShare";
import { useLocalVolume } from "./zoomHooks/useLocalVolume";
import "./video.scss";
import { isSupportWebCodecs } from "@/utils/platform";
import { isShallowEqual } from "@/utils/util";
import { useSizeCallback } from "@/hooks/useSizeCallback";
import { useAdvancedFeatureSwitch } from "./zoomHooks/useAdvancedFeatureSwith";
import RemoteControlPanel, {
  RemoteControlIndication,
} from "./zoomComponents/remote-control";
import { useCameraControl } from "./zoomHooks/useCameraControl";
import { useNetworkQuality } from "./zoomHooks/useNetworkQuality";
...
const VideoContainer: React.FunctionComponent = () => {
  ...
  const { advancedSwitch, toggleAdjustVolume, toggleFarEndCameraControl } =
    useAdvancedFeatureSwitch(zmClient, mediaStream, visibleParticipants);
  ...
  const onShareContainerResize = useCallback(
          ...
          {visibleParticipants.map((user, index) => {
            ...
            return (
              <Avatar
                ...
                volume={
                  userVolumeList.find((u) => u.userId === user.userId)?.volume
                }
